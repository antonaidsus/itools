...
public class SearchPanel extends JPanel implements ActionListener {
	...
	private void updateAutoCompleteDictionary() {
		...
		Collection values = Dictionary.getValues(name);
	...
	private void startSearch() {
		...
		LinkedList rscs = new LinkedList();
		if (pattern.trim().length() == 0 || category == null) {
			...
			Iterator i = displayer.getResources(searchAll.isSelected());
			pattern = Util.toJavaStyle(pattern);
			Pattern p = Pattern.compile(pattern, Pattern.CASE_INSENSITIVE);
			while (i.hasNext()) {
				NcbcResource rsc = (NcbcResource) i.next();
				String value = rsc.getProperty(category);
			...
			displayer.displayResources((List) rscs);
		}
