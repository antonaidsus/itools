...
public class NcbcResource implements Serializable {
	...
	public String[][] getOntology() {
		...
		for (int i = 0; i < tologies.length; i++) {
			rv[i] = tologies[i].split(Descriptor.DELIMITER);
		}
		return rv;
	}
	...
	/**
	 * return the first ontology if multiple ontologies available for this
	 * resource. starting from center, ending with its name;
	 * 
	 * @param rsc
	 * @return
	 */
	public String[] getFirstFullPath() {
		String[] strings = getOntology()[0];
		String[] rv = new String[strings.length + 2];
		rv[0] = getCenter();
		System.arraycopy(strings, 0, rv, 1, strings.length);
		rv[rv.length - 1] = getName();
		return rv;
	}
