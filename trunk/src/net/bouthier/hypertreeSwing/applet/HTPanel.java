...
import net.bouthier.hypertreeSwing.HTModelNode;
...
public class HTPanel extends JPanel implements ActionListener, PropertyChangeListener {
	...
	protected void createButtonsPanel() {
		...
		toogleCenterButton.setToolTipText("toogle centers info show/off in the view");
	...
	protected void createIcons() {
		homeIcon = GuiUtil.getIcon("/home.gif");		
		backIcon = GuiUtil.getIcon("/left.gif");
		dimmedBackIcon = GuiUtil.getIcon("/dimmedLeft.gif");
		forwardIcon = GuiUtil.getIcon("/right.gif");
		dimmedForwardIcon = GuiUtil.getIcon("/dimmedRight.gif");
		searchIcon = GuiUtil.getIcon("/search.gif");
		infoIcon = GuiUtil.getIcon("/info.gif");
		displayModelIcon = GuiUtil.getIcon("/displayModel.gif");
	}

	public void init() {
		removeAll();

		String value = HTVieweDataHelper.getAllResources4InteractiveViewer(isSandBox);
		...
		view.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				HTDrawNode drawNode = view.action.htDrawNode;
				if (drawNode == null) {
					logger.info("HTPanel mouseClicked:draw node is null");
					return;
				}
				...
				Controller.getInstance(isSandBox).firePropertyChange(new PropertyChangeEvent(HTPanel.this, Controller.SELECTED_PATH, null, 
						path.split(Descriptor.DELIMITER)));
	...
	protected void resetForSearch(HTXMLNode root) {
        ...
        foundHTDrawNodes.addElement((net.bouthier.hypertreeSwing.HTNode)root);
	}
	...
	public void findHT_Node(String nodeName) {
		...
		Enumeration enumNode = ((HTDrawNodeComposite) (view.draw.drawRoot))
				.children();
		...
		while (enumNode.hasMoreElements()) {
			...
			searchNode = (HTDrawNodeComposite) enumNode.nextElement();
			// if
	...
	public void selectNode(HTDrawNode node) {
		...
		Controller.getInstance(isSandBox).firePropertyChange(new PropertyChangeEvent(this, Controller.SELECTED_PATH, null, str.split(Descriptor.DELIMITER)));
		MainFrame.getContent().setStatus(str);
	}

	public void selectNode(String[] tokens) {
		view.draw.translateToOrigin(findDrawNode(tokens));
	}
	...
	public HTDrawNode findDrawNode(String[] tokens) {
		return findDrawNode(
				(net.bouthier.hypertreeSwing.HTDrawNodeComposite) view.draw.drawRoot,
				tokens, 0);
	}

	public HTDrawNodeComposite findDrawNode(HTDrawNodeComposite nodeComposite,
			String[] tokens, int index) {
		Enumeration enumNode = nodeComposite.children();
		String name = tokens[index++].trim();
		while (enumNode.hasMoreElements()) {
			HTDrawNodeComposite node = (HTDrawNodeComposite) enumNode
					.nextElement();
	...
	public void propertyChange(PropertyChangeEvent arg0) {
		...
		logger.info("PropertyEvent received: source=" +arg0.getSource() 
				+ ", property=" + propertyName +", nvalue=" + arg0.getNewValue());
